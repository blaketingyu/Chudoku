...
{					
	...
	if(isset($_POST["addproducts"])){
		...
		if (!empty($category)){
			...
			if(checkid($productID)==true){
				
			if(preg_match("/^([A-Za-z0-9\s])+[(]{1}+([A-Za-z0-9\s])+[)]{1}+$/",$productname)){
					...
					if (preg_match("/^[0-9]+$/",$unitsinstore)){
						if (preg_match("/^([a-zA-Z0-9\.\/\-])+$/",$picpath)){
							if (preg_match("/^([a-zA-Z0-9\.\/\-])+$/",$supplier)){
								if($_POST["addproducts"]=="yes")
										{	
											
											$query=$con->prepare("INSERT into products (productID,productName,productDescription,unitsPrice,unitsInStore,Picture,Supplier,Category) VALUES (?,?,?,?,?,?,?,?)");
											while ($query->fetch()){}
...
function err(errNo) {
    ...
    alert("ProductID already in use");
	window.location = 'addproducts.php';
	}
}
</script>