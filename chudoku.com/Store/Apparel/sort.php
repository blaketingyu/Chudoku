...
<html>
<head>
<link type="text/css" rel="stylesheet" href="stylesheetapparel.css"/>
</head>
...
<div class="refine">
	...
	<a href="sort.php?value=AttackOnTitan"><li style="font-family: Raleway" >Attack On Titan</li></a>
</div>
...
$sort=$_POST['Sort'];
	if($sort == "asc")
	{
 $query=$con->prepare("select * from products where category = 'Apparel' ORDER BY unitsPrice ASC");
 $query->execute();
...
if($sort == "desc")
{
$query=$con->prepare("select * from products where category = 'Apparel' ORDER BY unitsPrice DESC");
 $query->execute();
$query->bind_result($productid,$name,$description, $price, $units, $picture, $supplier, $category);
while($query->fetch())
{
...
if(isset($_GET['value'])){
  $query=$con->prepare("select * from products where category = 'Apparel' ");
 $query->execute();
$query->bind_result($productid,$name,$description, $price, $units, $picture, $supplier, $category);
